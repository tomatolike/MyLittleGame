...
# This is the main logic file of the game
# Each function represent a different game logic
...
class Game:
	...
	# This function is used to initialize the 'thing' object in the world
	# 'thing' object are mainly the equipments or tools that can be used or exchanged by users and NPCs
	def GenerateThings(self):
		...
		while True:
			...
			if x == "NEW":
				...
				# Protect type things are equipments that can be used to protect the user and NPCs, to increase protection
				if x == "Protect":
					new = Protect()
					x = f.readline().strip('\n') # Read the id
					new.id = int(x)
				...
				# Weapon type things to increase attack.
				elif x == "Weapon":
					new = Weapon()
					x = f.readline().strip('\n') # Read the id
					new.id = int(x)
				...
				# Things that are not kind of equipments. Usually have some special meaning.
				# Used to fullfil the plot, like a diamond that can make the prince fall in love with you.
				elif x == "Thing":
					new = Thing()
					x = f.readline().strip('\n') # Read the id
					new.id = int(x)
	...
	# Find a special object by the object-id
	def findObj(self,x):
		for a in Objects.objlist:
			if a.id == x:
				return a

	# This function is used to initialize the 'Action' in the game world
	# 'Action' is the logic of NPCs, like how they fight, how they communicate and how they exchange things
	# They are like logic rules which can be modified or generated by users.
	# We can create NPCs! Yes! Although, now, the logic rules are writen in code, maybe we can add some randomness or machine learning to make them act like a real person.
    ...
    # This function is used to generate 'Actor' in the world
	# The 'Actor' include both NPCs and users.
	# This function will not only generate the 'Actor' objects, but also attach the 'Thing' objects and 'Action' objects to them.
	# For example, suppose I am a 'Actor' who is a fighter, this function will attach the 'knife' object to me.
	...
	# This function is used to generate the 'Place' in the world.
	# The system of these 'Place' objects is just like a tree data structure.
	# For example, 'Like's Home' is the son of 'Chengdu'.
	...
	# Save the 'Thing' objects into files
	def saveThing(self):
		print("Saving Things!")
		f = open('things.txt','w')
	...
	# Save the 'Action' objects into files
	def saveAction(self):
	...
	# Save the 'Place' objects into files
	def savePlaces(self):
		print("Saving Places!")
		f = open('places.txt','w')
	...
	# Save the 'Actors' objects into files
	def saveActors(self):
		print("Saving Actors!")
		f = open('actors.txt','w')
		for i in Actor.actorlist:
	...
	# This function is used to send message to clients
	def prints(self, a, s):
		a.net.prints(s)

	# This function is used to receive message from clients
	def inputs(self, a, s=""):
		return a.net.inputs(s)

	# This function is used when a user start communication with an NPC
	# Which means the NPC will follow the 'Action' he has to communicate with the user
	def GetInLogic(self, a, b):
		print(time.strftime("%Y-%m-%d %H:%M:%S: ", time.localtime())+a.name+" and "+b.name+" GetInLogic!")
		a.busy.acquire() # Busy lock locked
		if b.logic == None: # logic=None means the NPC has no 'Action'.
			self.prints(a,b.name+"是一个沉默寡言的人。")
			a.busy.release() # Busy lock released
			return
		if not (b.logic.orders): # Also means the NPC has no 'Action'.
			self.prints(a,b.name+"是一个沉默寡言的人。")
			a.busy.release()
			return
